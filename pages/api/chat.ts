...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  function generateDemoReply(userText: string): string {
    ...
    if (/(urgent|asap|emergency|quick|fast)/.test(text)) {
      return "I understand this is urgent! We can often provide same-day support for critical issues. For immediate assistance, please email info@probooksolutions.org with 'URGENT' in the subject line, or book a consultation and mention it's urgent. What's the specific issue you need help with?";
    }
